...
import com.sgahs.springboot.app.models.dao.service.ICustomMovimientoService;
...
public class DashboarController {
	...
	private static final String DETALLE_GASTO = "detalleGasto";
	...
	private static final String FORM_GASTO = "formGasto";
	...
	private static final String MONTO_GASTADO_MES_ACTUAL = "montoGastado";
	...
	public Double calcularTotal(String movimiento) {
		...
		case GASTO_DISPONIBLE:
			total = customMovimientoService.obtenerGastoDisponible();
			break;
		case MONTO_GASTADO_MES_ACTUAL:
			total = customMovimientoService.obtenerGastoMensual();
			break;
		default:
			total = 0.0;
		}
	 ...
	 *                                    vista
	 * @param isGastoReqired              booleano que indica si requiere el objeto
	 *                                    Gasto()
	 * @param gasto                       instancia del objeto Gasto(), si es nulo
	 *                                    crea una instancia
	 * @param opcion                      indica la vista que debera ser mostrada al
	 *                                    usuario
	 * @param isDetalle                   boleano que indica si se debe obtener la
	 *                                    lista de gastos en la BD
	 * @param isGastosRecurrentesRequired boleano que indica si se requiere obtener
	 *                                    la lista de gastos recurrentes
	 */
	public void initModelGasto(Model model, boolean isGastoReqired, Gasto gasto, String opcion, boolean isDetalle,
			boolean isGastosRecurrentesRequired) {
		...
		if (isDetalle) {
			List<Gasto> gastos = movimientoService.findAllGasto();
			model.addAttribute("gastos", gastos);
		}

		if (isGastosRecurrentesRequired) {
			List<GastoRecurrente> gastosRecurrentes = movimientoService.findAllGastoRecurrente();
			model.addAttribute("gastosRecurrentes", gastosRecurrentes);
		}

		model.addAttribute(TOTAL_GASTO_DISPONIBLE, calcularTotal(GASTO_DISPONIBLE));
		model.addAttribute(MONTO_GASTADO_MES_ACTUAL, calcularTotal(MONTO_GASTADO_MES_ACTUAL));
	}

	@GetMapping("/detalleGasto")
	public String obtenerDetalleGasto(Model model) {

		initModelGasto(model, false, null, DETALLE_GASTO, true, false);
	...
	public String formGasto(Model model) {
		initModelGasto(model, true, null, FORM_GASTO, false, true);
	...
	public String agregarGasto(@Valid Gasto gasto, BindingResult result, Model model, RedirectAttributes flash) {
		...
		Double montoDisponible = calcularTotal(GASTO_DISPONIBLE);

		if (result.hasErrors() || gasto.getMonto() > montoDisponible) {
			initModelGasto(model, true, gasto, FORM_GASTO, false, true);
		...
		movimientoService.saveGasto(gasto);
	...
	public String agregarInversion(@Valid Inversion inversion, BindingResult result, Model model,
		...
		if (folio == null || folio.isEmpty()) {
			...
			if (inversion.getMontoRetirado() == null || inversion.getMontoRetirado() == 0) {
				initModelInversion(model, true, inversionAnterior, FORM_INVERSION, false, ACTUALIZA, false);
				model.addAttribute(HAS_MESSAGE_ERROR, true);
